...
int main(int argc, char **argv) {
  ...
  int frame_rate;
  nh_params.param<int>("framerate", frame_rate, 120);

  ros::Publisher pub = node.advertise<sensor_msgs::Imu>("imu", frame_rate);
  ros::Rate rate(frame_rate);  // hz

  ROS_INFO("Beginning IMU data publishing at %d hz", frame_rate);
  ...
  while(ros::ok()) {
    ...
    pub.publish(msg);
    ros::spinOnce();
    // todo: does this results to lower than set freq due to processing time?
    rate.sleep();
  }
  return 0;
}
